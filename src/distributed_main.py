...
def train_resnet(gpu, arguments):
    dist.init_process_group(backend='nccl', init_method='env://', world_size=arguments.gpu_nums, rank=gpu)
    torch.cuda.set_device(gpu)
    ...
    model, start_epochs = utils.load_weights(model, model_directory, arguments.checkpoint)
    optimizer = torch.optim.AdamW(model.parameters(), lr=arguments.lr, amsgrad=True)
    # optimizer = torch.optim.SGD(model.parameters(), lr = arguments.lr, nesterov=True, weight_decay=1e-5, momentum=0.9)
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.1, patience=5, min_lr=0.001, verbose=True)
    # criterion = torch.nn.CTCLoss(reduction='sum', zero_infinity=True)
    ...
    for epoch in range(start_epochs, arguments.num_epochs):
        ...
        for idx, (images, labels, filepaths) in enumerate(progress_bar):
            ...
            with amp.autocast():
                predictions = model(train_images)
                logits = predictions.log_softmax(2)
                input_len, b_size, vocab_size = logits.size()
                prediction_size = torch.LongTensor([predictions.size(0)]).repeat(batch_size)
                stop = 1
                logits_lens = torch.full(size=(batch_size,), fill_value=input_len, dtype=torch.int32)
                train_current_loss = criterion(logits, encoded_train_labels, logits_lens, length)
            predictions = logits.argmax(2).detach().cpu()
            predicted_train_labels = np.array(converter.decode(predictions, prediction_size, raw=False))
            correct_train_predictions += (predicted_train_labels == labels).sum()
            # word_error_rate += wer_metric.wer(predicted_train_labels, labels)
            train_mean_loss = train_mean_loss * (
            ...
            total_train_images += batch_size
            epoch_desc = '[{}/{}] Current Loss: {:.4f} Loss: {:.4f} Acc: {:.4f}'.format(epoch, config.epochs,
                                                                                        train_current_loss,
                                                                                        train_mean_loss,
                                                                                        correct_train_predictions / total_train_images)
                ...
                with torch.no_grad():
                    for val_images, val_labels, val_filepaths in val_dataloader:
                        ...
                        with amp.autocast():
                            ...
                            stop = 1
                            prediction_size = torch.LongTensor([predictions.size(0)]).repeat(batch_size)
                            current_val_loss = criterion(logits, encoded_val_labels, logits_lens, length)
                        predictions = logits.argmax(2).detach().cpu()
                        predicted_val_labels = np.array(converter.decode(predictions, prediction_size, raw=False))
                        correct_val_predictions += (predicted_val_labels == val_labels).sum()
                        val_mean_loss = val_mean_loss * (total_val_images / (
...
def train_lite(gpu, arguments):
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=arguments.lr, amsgrad=True)
    # optimizer = torch.optim.SGD(model.parameters(), lr = arguments.lr, nesterov=True, weight_decay=1e-5, momentum=0.9)
    # optimizer = torch.optim.Adadelta(model.parameters(), lr=arguments.lr)
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.1, patience=5, min_lr=0.001, verbose=True)
...
def train_crnn3(gpu, arguments):
    ...
    model, start_epochs = utils.load_weights(model, model_directory, arguments.checkpoint)
    optimizer = torch.optim.AdamW(model.parameters(), lr=arguments.lr, amsgrad=True)
    scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.1, patience=5, min_lr=0.0000001, verbose=True)
    criterion = torch.nn.CTCLoss(reduction='sum', zero_infinity=True)
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
    converter = StrLabelConverter(config.alphabet)
    ...
    for epoch in range(start_epochs, arguments.num_epochs):
        ...
        for idx, (images, labels, filepaths) in enumerate(progress_bar):
            ...
            with amp.autocast():
                predictions = model(train_images)
                log_probs = predictions.log_softmax(2)
                prediction_size = torch.LongTensor([predictions.size(0)] * batch_size)
                length = torch.flatten(length)
                train_current_loss = criterion(log_probs, encoded_train_labels, prediction_size, length)
            predictions = log_probs.argmax(2).detach().cpu()
            predicted_train_labels = np.array(converter.decode(predictions, prediction_size, raw=False))
            correct_train_predictions += (predicted_train_labels == labels).sum()
            word_error_rate += wer_metric.wer(predicted_train_labels, labels)
            train_mean_loss = train_mean_loss * (
                ...
                with torch.no_grad():
                    for val_images, val_labels, val_filepaths in val_dataloader:
                        ...
                        with amp.autocast():
                            ...
                            prediction_size = torch.LongTensor([predictions.size(0)] * batch_size)
                            length = torch.flatten(length)
                            current_val_loss = criterion(log_probs, encoded_val_labels, prediction_size, length)
                        predictions = log_probs.argmax(2).detach().cpu()
                        predicted_val_labels = np.array(converter.decode(predictions, prediction_size, raw=False))
                        correct_val_predictions += (predicted_val_labels == val_labels).sum()
                        val_mean_loss = val_mean_loss * (total_val_images / (
...
def train_crnn3_cls(gpu, arguments):
    dist.init_process_group(backend='nccl', init_method='env://', world_size=arguments.gpu_nums, rank=gpu)
    torch.cuda.set_device(gpu)
    ...
    for epoch in range(start_epochs, arguments.num_epochs):
        ...
        for idx, (images, labels, region, filepaths) in enumerate(progress_bar):
            ...
            epoch_desc = '[{}/{}] Current Loss: {:.4f} Loss: {:.4f} Acc: {:.4f} Cls Acc:{:.4f} WER: {:.4f}'.format(
                epoch, config.epochs, train_current_loss, train_mean_loss,
                correct_train_predictions / total_train_images, correct_cls_predictions / total_train_images, 0)
            i += 1
            if gpu == 0:
                # logger.info(epoch_desc)
                progress_bar.set_description(epoch_desc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='/data_sda/weights')
    parser.add_argument('--data_dir', type=str, default='/data_sda')
    parser.add_argument('--model_name', type=str, default='/azer_crnn_lite')
    ...
    random.seed(42)
    np.random.seed(42)
    mp.spawn(train_crnn3, nprocs=args.gpu_nums, args=(args,))
